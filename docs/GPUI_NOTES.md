...
6. **窗口图标**:`WindowOptions` 无 icon 字段(全字段已核对)。macOS 应用图标属于 .app bundle(Info.plist/Assets),开发期 cargo run 出来的二进制没有图标是正常的;发布需要打包 .app(可用 cargo-bundle 等,**未验证,脚手架阶段再选型**)。
