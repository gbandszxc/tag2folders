...
6. **窗口图标**:`WindowOptions` 无 icon 字段(全字段已核对)。macOS 应用图标属于 .app bundle(Info.plist/Assets),开发期 cargo run 出来的二进制没有图标是正常的。**已落地:`scripts/build-dmg.sh`**(纯系统工具 hdiutil/iconutil/codesign,不引入 cargo-bundle)完成 release 二进制 → .app(Info.plist + `assets/AppIcon.icns`,图标源 `assets/app-icon.svg`)→ DMG 全流程,2026-08-22 已实测出包并挂载运行验证。
